...
def grep_keywords(keywordList,RepoDir):
    ...
    for key,value in keywordDict.items():
        ...
        key = key.replace(' ','')
        print(key)
        command = "cd " + RepoDir + "&&" + "git log --grep '" + value + "' -E | grep '^commit' > /home/seclab/dingzhu/gitpro/keySearchinGit/commits/" + key + ".commits"
        os.system(command)
    ...
    # logNub = print_res_of_target_keyword(targetList) 
    # print(logNub)
...
def update(keywordList,newRepoDir): 
    ...
    for key,value in keywordDict.items():
        ...
        key = key.replace(' ','')
        print(key)
        command = "cd " + RepoDir + "&&" + "git log --grep '" + value + "' -E | grep '^commit' > /home/seclab/dingzhu/gitpro/keySearchinGit/update/commits/" + key + ".commits"
        os.system(command)
...
def main():
    ...
    # colledt targetkeywords and count them
    targetDict = keywordList[2]
    for key,value in targetDict.items():
        targetList = value
